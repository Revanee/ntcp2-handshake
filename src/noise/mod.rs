...
pub fn dh(key_pair: KeyPair, public_key: Key) -> Key {
...
pub fn encrypt(k: Key, n: u64, ad: &[u8], plaintext: &[u8]) -> Vec<u8> {
...
pub fn hmac_hash(key: Key, data: &[u8]) -> [u8; HASHLEN] {
    use hmac::Mac;
...
pub fn hkdf(
    ...
    let output2 = hmac_hash(temp_key, [output1.as_slice(), &[2u8]].concat().as_slice());
    if num_outputs == 2 {
        return (output1, output2, None);
    }
    let output3 = hmac_hash(temp_key, [output2.as_slice(), &[3u8]].concat().as_slice());
